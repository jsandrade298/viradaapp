...
export default function FeedScreen() {
  ...
  // STORIES — usando refs para evitar stale closures nos timers
  ...
  // Timer via setTimeout (não depende de callback do Animated)
  ...
  async function sendComment() {
    ...
    const { data, error } = await supabase.from('comments').insert({
      post_id: commentModalPost.id, author_id: currentUser.id, content: newComment.trim(),
    }).select('*, profiles(full_name, avatar_url)').single();
    setSendingComment(false);
    if (!error && data) {
      setComments(prev => [...prev, data]);
      setNewComment('');
      // Atualizar contagem no feed
  ...
  // ========== STORIES: CONTROLE VIA setTimeout + Animated (visual only) ==========
  ...
  function prevStory() {
    ...
    if (si > 0) {
      ...
      groupRef.current = pg;
      storyRef.current = lastIdx;
      setActiveGroupIdx(pg);
  ...
  // Ordenar stories: global primeiro, depois não-vistos, depois vistos
  function getSortedStoryGroups(): StoryGroup[] {
    const globalGroup = storyGroups.filter(g => g.isGlobal);
    const unseen = storyGroups.filter(g => !g.isGlobal && !viewedGroups.has(g.sourceId));
    const seen = storyGroups.filter(g => !g.isGlobal && viewedGroups.has(g.sourceId));
    return [...globalGroup, ...unseen, ...seen];
  }

  // ========== RESET / MEDIA ==========
  function resetPostModal() {
    setNewPostContent(''); setAttachment(null); setLinkUrl(''); setShowLinkInput(false);
    setPollOptions([]); setShowPollEditor(false); setPostLocation(null); setIsPostModalVisible(false);
  }
  ...
  async function pickImage(src: 'camera' | 'gallery') {
    if (src === 'camera') {
      const p = await ImagePicker.requestCameraPermissionsAsync(); if (!p.granted) { Alert.alert("Permissão negada"); return; }
      const r = await ImagePicker.launchCameraAsync({ mediaTypes: ['images'] as any, allowsEditing: true, quality: 0.7 });
      if (!r.canceled) setAttachment({ uri: r.assets[0].uri, type: 'image', mimeType: 'image/jpeg' });
    } else {
      const r = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ['images'] as any, allowsEditing: true, quality: 0.7 });
      if (!r.canceled) setAttachment({ uri: r.assets[0].uri, type: 'image', mimeType: r.assets[0].mimeType || 'image/jpeg' });
    }
  }
  async function pickGif() {
    const r = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ['images'] as any, quality: 1 });
    if (!r.canceled) setAttachment({ uri: r.assets[0].uri, type: 'gif', mimeType: 'image/gif' });
  }
  async function pickVideo(src: 'camera' | 'gallery') {
    if (src === 'camera') {
      const p = await ImagePicker.requestCameraPermissionsAsync(); if (!p.granted) { Alert.alert("Permissão negada"); return; }
      const r = await ImagePicker.launchCameraAsync({ mediaTypes: ['videos'] as any, allowsEditing: true, videoMaxDuration: 120, quality: 0.7 });
      if (!r.canceled) setAttachment({ uri: r.assets[0].uri, type: 'video', mimeType: r.assets[0].mimeType || 'video/mp4' });
    } else {
      const r = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ['videos'] as any, allowsEditing: true, videoMaxDuration: 120, quality: 0.7 });
      if (!r.canceled) setAttachment({ uri: r.assets[0].uri, type: 'video', mimeType: r.assets[0].mimeType || 'video/mp4' });
    }
  }
  async function pickDocument() {
    try {
      const r = await DocumentPicker.getDocumentAsync({ type: ['application/pdf','application/msword','application/vnd.openxmlformats-officedocument.wordprocessingml.document','application/vnd.ms-excel','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','text/plain'], copyToCacheDirectory: true });
      if (!r.canceled && r.assets?.[0]) { const d = r.assets[0]; setAttachment({ uri: d.uri, type: 'document', mimeType: d.mimeType || 'application/octet-stream', fileName: d.name }); }
    } catch { Alert.alert("Erro", "Não foi possível selecionar o documento."); }
  }
  ...
  function addPollOption() { pollOptions.length < 5 ? setPollOptions([...pollOptions, '']) : Alert.alert("Limite", "Máximo 5 opções."); }
  function removePollOption(i: number) { pollOptions.length <= 2 ? Alert.alert("Mínimo", "Pelo menos 2.") : setPollOptions(pollOptions.filter((_,j) => j !== i)); }
  async function handleLocationToggle() {
    if (postLocation) { setPostLocation(null); return; }
    setIsLoadingLocation(true);
    try {
      const { status } = await Location.requestForegroundPermissionsAsync(); if (status !== 'granted') { Alert.alert("Permissão negada"); setIsLoadingLocation(false); return; }
      ...
      if (addr) { const p = [addr.street, addr.district, addr.city].filter(Boolean); name = p.join(', ') || addr.name || name; }
      setPostLocation({ name, latitude: loc.coords.latitude, longitude: loc.coords.longitude });
  ...
  async function uploadMedia(uid: string) {
    ...
    const ct = attachment.mimeType || (attachment.type === 'video' ? 'video/mp4' : attachment.type === 'gif' ? 'image/gif' : attachment.type === 'document' ? 'application/octet-stream' : 'image/jpeg');
  ...
  async function handleCreatePost() {
    ...
    if (showPollEditor && pollOptions.filter(o => o.trim()).length < 2) { Alert.alert("Enquete incompleta"); return; }
    if (showLinkInput && linkUrl.trim() && !/^https?:\/\//i.test(linkUrl.trim())) { Alert.alert("Link inválido"); return; }
    setIsPublishing(true);
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) { Alert.alert("Erro"); setIsPublishing(false); return; }
  ...
  async function handlePollVote(pid: string, idx: number, opts: PollOption[]) {
    if (userVotes[pid] !== undefined) return;
    const upd = opts.map((o, i) => ({ ...o, votes: i === idx ? o.votes + 1 : o.votes }));
    const { error } = await supabase.from('posts').update({ poll_options: upd }).eq('id', pid);
    if (!error) { setUserVotes(p => ({ ...p, [pid]: idx })); fetchFeed(); }
  }
  ...
  function renderPostPoll(item: any) {
    ...
    return <View className="mb-4">{opts.map((o, i) => {
      ...
      return <TouchableOpacity key={i} onPress={() => handlePollVote(item.id, i, opts)} disabled={voted} activeOpacity={voted ? 1 : 0.7} className={`mb-2.5 rounded-xl overflow-hidden border ${sel ? 'border-[#ff4500]' : 'border-zinc-700'}`}>
    ...
    })}<Text className="text-gray-500 text-xs mt-1 ml-1">{total} {total === 1 ? 'voto' : 'votos'}{voted ? ' · Você votou' : ' · Toque para votar'}</Text></View>;
  }
  ...
  return (
    <View className="flex-1 bg-[#1a1a1a]">
          ...
          <View className="mb-6">
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row">
              {sortedGroups.map(group => {
                // Buscar índice real no array original para openStoryGroup
            ...
            <View className="flex-row items-center justify-between border-t border-zinc-800 pt-4 px-1">
              <View className="flex-row items-center">
                <TouchableOpacity onPress={() => toggleInteraction(item.id, 'APOIAR')} className="flex-row items-center mr-7">
                  <Ionicons name={hasInt(item.id, 'APOIAR') ? 'flame' : 'flame-outline'} size={22} color={hasInt(item.id, 'APOIAR') ? '#ff4500' : '#9ca3af'} />
                  <Text className={`text-sm font-medium ml-1.5 ${hasInt(item.id, 'APOIAR') ? 'text-[#ff4500]' : 'text-gray-400'}`}>{item.apoiar_count||0}</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => openComments(item)} className="flex-row items-center mr-7">
                ...
                <TouchableOpacity onPress={() => toggleInteraction(item.id, 'REPERCUTIR')} className="flex-row items-center">
                  <Ionicons name={hasInt(item.id, 'REPERCUTIR') ? 'repeat' : 'repeat-outline'} size={22} color={hasInt(item.id, 'REPERCUTIR') ? '#10b981' : '#9ca3af'} />
                  <Text className={`text-sm font-medium ml-1.5 ${hasInt(item.id, 'REPERCUTIR') ? 'text-emerald-500' : 'text-gray-400'}`}>{item.repercutir_count||0}</Text>
                </TouchableOpacity>
              </View>
      ...
      {/* ===== MODAL COMENTÁRIOS ===== */}
      <Modal visible={!!commentModalPost} animationType="slide" transparent onRequestClose={() => { setCommentModalPost(null); setComments([]); setNewComment(''); }} statusBarTranslucent>
        <View className="flex-1 bg-black/60 justify-end">
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="bg-zinc-900 rounded-t-3xl border-t border-zinc-700" style={{ maxHeight: SH * 0.85 }}>
            {/* Handle + Header */}
            <View className="pt-3 pb-2 items-center border-b border-zinc-800">
              <View className="w-10 h-1 bg-zinc-600 rounded-full mb-3" />
              <View className="flex-row items-center justify-between w-full px-5 pb-2">
                <Text className="text-white font-bold text-lg">Comentários</Text>
                <TouchableOpacity onPress={() => { setCommentModalPost(null); setComments([]); setNewComment(''); }}><Ionicons name="close" size={24} color="#9ca3af" /></TouchableOpacity>
              </View>
            </View>
              ...
              <FlatList
                ...
                renderItem={({ item: c }) => (
                  <View className="flex-row mb-4">
                    <View className="w-9 h-9 bg-zinc-700 rounded-full items-center justify-center mr-3 mt-0.5">
                      <Text className="text-white font-bold text-sm">{c.profiles?.full_name?.charAt(0)||'?'}</Text>
                    </View>
                    <View className="flex-1 bg-zinc-800 rounded-2xl rounded-tl-sm px-4 py-3">
                      ...
                      <Text className="text-gray-300 text-sm leading-5">{c.content}</Text>
                    </View>
                  </View>
                )}
              />
            ...
            {/* Input de comentário */}
            <View className="flex-row items-center px-4 py-3 border-t border-zinc-800 pb-8">
          ...
          {activeGroup && activeStory && (
            <>
              ...
              <View className="flex-1">
                ...
                {/* Zonas de toque com long press */}
                <View className="absolute inset-0 flex-row" pointerEvents="box-none">
                  <Pressable onPress={prevStory} onLongPress={pauseStory} onPressOut={() => { if (pausedRef.current) resumeStory(); }} delayLongPress={150} style={{ flex: 1 }} />
                  <Pressable onPress={advanceStory} onLongPress={pauseStory} onPressOut={() => { if (pausedRef.current) resumeStory(); }} delayLongPress={150} style={{ flex: 2 }} />
                </View>

                {paused && <View className="absolute top-4 left-0 right-0 items-center"><View className="bg-black/50 px-3 py-1 rounded-full flex-row items-center"><Ionicons name="pause" size={12} color="white" /><Text className="text-white text-xs font-medium ml-1">Pausado</Text></View></View>}
              </View>

              {storyGroups.length > 1 && <View className="flex-row justify-center pb-10 gap-1.5">{storyGroups.map((_, i) => <View key={i} className={`w-1.5 h-1.5 rounded-full ${i === activeGroupIdx ? 'bg-white' : 'bg-zinc-600'}`} />)}</View>}
            </>
          )}
        </View>
      </Modal>
